...
onready var turret_animation = $Body
...
func apply_movement():
	if velocity.x != 0 :
		turret_animation.flip_h = velocity.x == -1
	velocity.y += 20
...
func _play_animation(anima_string):
	turret_animation.play(anima_string)
...
func _on_Body_animation_finished():
	if turret_animation.animation == "Dead" :
		hide()
		collision_layer = 0
		collision_mask = 0
